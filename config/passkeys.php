...
return [
    ...
    */
    ...
    // The package's own default is ['password.confirm'], but this app has
    // no password-confirmation flow at all yet (no route named
    // 'password.confirm', no confirm-password page) — for ANY action, not
    // just passkeys. Wiring that up is a separate, app-wide piece of auth
    // infrastructure this task didn't ask for, so management is gated by
    // plain `auth` (the surrounding route group) only for now. Revisit if
    // stronger re-authentication before adding/removing a passkey is
    // wanted later.
