...
| Job milestone percentage (Phase 03) | Regression: the "Amount is a percentage of job value" toggle used to accept a `percentage` that nothing ever turned into `amount`, so `ApproveSalesOrder`'s total check (sums `amount` only) silently required typing the amount by hand anyway — `percentage` now live-computes and locks `amount` via the relation manager's form, and the table shows the recorded percentage | `MilestonesRelationManagerTest` |
