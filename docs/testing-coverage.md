...
| Vendor price list import (`import:pricelist` + Price List's "Import pricelist" action) | Header-detection parser against a synthetic workbook (stacked product families, category-label tracking, reference-price tier heuristic, re-import upserts not duplicates) — see `docs/price-list-import.md` for the real-file verified row counts, not reproducible in an automated test since real vendor pricelists are never committed; the admin-panel Import action and "Create/update product" sync (`App\Services\ProductSync`) wired end-to-end | `PriceListImporterTest`, `PriceListItemResourceTest`, `ProductSyncTest` |
