    ...
    <div class="max-w-[794px] w-full mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white dark:bg-gray-900/40 border border-gray-200 dark:border-gray-800 rounded-lg p-4">
        @if ($isPreview)
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center w-full">
                ...
                <div class="flex flex-col items-start sm:items-end gap-1">
                    <x-button text="Generate & Issue" icon="document-check" color="green" sm class="h-9" wire:click="generateAndIssue" wire:confirm="Generate this Statement of Account? This becomes a permanent numbered record — cannot be un-generated." loading="generateAndIssue" spinner="dots" />
        ...
        @else
            ...
            <div class="flex items-center gap-2">
                <x-button text="Download PDF" icon="document-arrow-down" color="gray" sm class="h-9" href="{{ route('statement-of-accounts.pdf', $soa->id) }}" target="_blank" />
                <x-button text="Email to client" icon="envelope" color="blue" sm class="h-9" wire:click="emailToClient" loading="emailToClient" spinner="dots" />
            </div>
        @endif
    </div>
</div>
