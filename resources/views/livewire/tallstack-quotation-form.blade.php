<div class="w-[93%] mx-auto py-6 flex flex-col gap-5">
    ...
    {{--
        Layout: line items and notes are this page's primary content, so
        they get full page width in their own stacked rows below — not
        squeezed into a narrow lg:grid-cols-[1.6fr_1fr] column fighting a
        sidebar (the problem on the current Invoice edit page this
        pattern deliberately avoids). The two "basic info" cards (client/
        terms, financial summary) sit in a compact top row instead and
        collapse via <x-card minimize> once a quotation already exists —
        the header's meta row above keeps client/date/total visible at a
        glance even while collapsed, so nothing is genuinely hidden. A
        brand-new quotation (no record yet, fields still need filling)
        never gets minimize — there's nothing to collapse into a summary
        of yet.
    --}}
    <div class="grid lg:grid-cols-2 gap-4 items-start">
        {{-- Client & terms --}}
        <x-card :minimize="$quotation ? 'mount' : null">
            <x-slot:header>
    ...
    {{-- Line items — the primary content of this page, full width. --}}
    <x-card>
        <x-slot:header>
    ...
    {{-- Notes — the other primary content of this page, full width. --}}
    <x-card>
        <x-slot:header>
    ...
    {{-- Secondary/reference cards — Customer PO/COC and Job link, only
         when they apply, then Documents. Kept below the primary content
         rather than a competing sidebar. --}}
    ...
    {{-- Documents — see App\Livewire\Concerns\ManagesDocuments; same shape as the Invoice form's own Documents card. --}}
    @if ($quotation)
        <x-card>
            <x-slot:header>
